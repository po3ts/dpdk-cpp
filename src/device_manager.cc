...
dpdkpp::device_manager_t::device_manager_t(unsigned int nb_elements, unsigned int cache_size)
    : mbuf_pool{nullptr},
      devices{} {
...
dpdkpp::device_manager_t::~device_manager_t() { cleanup(); }

// Free resources
void dpdkpp::device_manager_t::cleanup() {
    if (mbuf_pool == nullptr) {
        return;
    }
